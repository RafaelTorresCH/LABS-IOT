...
## Alcance Lab2: sensores -> gateway

Este proyecto queda preparado para la parte que nos corresponde del sistema:
los nodos ESP32-C6 publican datos por MQTT en la red Wi-Fi local y el gateway
ESP32-C6 reenvia esos mensajes por CoAP hacia el border router OpenThread del
otro equipo.

### Pines usados en los nodos ESP32-C6

| Funcion | GPIO | Tipo |
| --- | --- | --- |
| DHT11/DHT22 DATA | GPIO18 | Digital con pull-up interno |
| Humedad de suelo AO | GPIO0 | Analogico ADC |
| Nivel de agua S/AO | GPIO1 | Analogico ADC o digital |
| Control bomba | GPIO19 | Salida digital |

Estos valores tambien estan centralizados en `main/sensor_gateway_config.h`:
`LAB2_DHT_DATA_GPIO`, `LAB2_SOIL_MOISTURE_ADC_GPIO`,
`LAB2_WATER_LEVEL_GPIO` y `LAB2_PUMP_CONTROL_GPIO`.

### Contrato MQTT -> CoAP

El gateway se suscribe a:

```text
lab2/sensors/+/telemetry
lab2/actuators/+/+/set
```

Ejemplos esperados desde los nodos:

```text
lab2/sensors/nodo1/telemetry
{"temperature_c":24.8,"air_humidity_pct":58.0,"soil_humidity_pct":42.5}

lab2/sensors/nodo2/telemetry
{"water_level_pct":71.0,"pump":0}

lab2/actuators/nodo2/pump/set
{"pump":1}
```

El gateway traduce esos mensajes a CoAP:

```text
POST /sensors/{nodo}/telemetry
PUT  /actuators/{nodo}/{actuador}
```

Antes de flashear el gateway, ajustar en `main/sensor_gateway_config.h`:
`LAB2_WIFI_SSID`, `LAB2_WIFI_PASSWORD`, `LAB2_MQTT_BROKER_URI` y
`LAB2_BORDER_ROUTER_HOST`.
